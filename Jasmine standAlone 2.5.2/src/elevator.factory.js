...
factory('Elevator', ['Floors', function(floors) {
		...
		Car.prototype = {
			...
			shutCarDoor: function() {
				...
				//             if (angular.isDefined($scope.nDeserved) && angular.isDefined(toDeserve[0])){
				// $scope.nDeserved == toDeserve[0] ? (deserveStatement = true, $scope.timer()) : deserveStatement = false;
				// }
			},
